...
import { checkAvailableStock } from '../store/actions/productActions';
    ...
    <SafeAreaView style={styles.safeAreaContainer}>
      <ScrollView>
        ...
        {cartItems.length === 0 ? (
          ...
          cartItems.map(item => (
            <View style={styles.container} key={item.data_id}>
              <TouchableOpacity onPress={() => navigation.navigate('DetailProductScreen', { productId: item.product_id })}>
                <Image source={{ uri: item.image }} style={styles.productImage} />
                <Text style={styles.productName}>{item.name}</Text>
                {/* <Text>{item.reservation_expires_at}</Text> */}
              </TouchableOpacity>
              ...
              <View style={styles.quantityPicker}>
                ...
                >
                  {[...Array(item.reserved_qty).keys()].map((x) => (
